...
function __asyncjs__js_delay(ms) { return Asyncify.handleAsync(async () => { await new Promise(function(resolve) { setTimeout(resolve, ms); }); }); }
__asyncjs__js_delay.sig = 'vi';
...
var _run_async = Module['_run_async'] = createExportWrapper('run_async', 1);
...
(() => {
  // Create a small, never-async wrapper around JspiModule which
  // checks for callers incorrectly using it with `new`.
  var real_JspiModule = JspiModule;
  JspiModule = function(arg) {
    if (new.target) throw new Error("JspiModule() should not be called with `new JspiModule()`");
    return real_JspiModule(arg);
  }
})();
if (typeof exports === 'object' && typeof module === 'object') {
  module.exports = JspiModule;
  // This default export looks redundant, but it allows TS to import this
  // commonjs style module.
  module.exports.default = JspiModule;
