...
// Navigation via `location.href = ...` from the page's own script. The
// ergonomic upgrade of self_redirect.test.mjs: that test only proves the
// session survives an uncontrolled self-redirect (no crash). This proves the
// full contract — the buffered breakpoint refires on the new page and
// continue() keeps working — now that navigation re-syncs the debug session
// instead of just not crashing it.
...
test("location.href assignment navigates and the breakpoint refires on the new page", async () => {
  s.evaluate("assignPage2()");
