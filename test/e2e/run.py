...
# ---- threaded (pthreads / web workers) tests ---------------------------
# Uses examples/threaded (pre-built emscripten pthreads matmul).
# The static server already sends COOP/COEP so SharedArrayBuffer works.

_THREADED = {
    "page_dir": "test/e2e/fixtures/threaded",
    # runMatmul() is a global in the page that calls matmul_threaded(4).
    "fire": "runMatmul()",
}
...
class TestThreaded(TestBase):
    def _start(self, timeout=120):
        ...
        Emscripten's idle pool workers are in JS Atomics.wait and do not fire
        wasm breakpoints; we break on the main-thread function instead. The
        pool workers are still visible in the thread list and are interrupted
        by all-stop when the main thread hits the breakpoint.
        """
        platform_port = self._start_platform(_THREADED, timeout=timeout)
        target, process = self._connect_via_platform(platform_port)
        # matmul_threaded runs on the main JS thread (via runMatmul()), so its
        # breakpoint fires reliably even in all-stop mode.
    ...
    def test_param_is_accessible(self):
        ...
        # nthreads = 4 (from runMatmul() calling matmul_threaded(4))
    ...
    def test_continue_after_stop(self):
        ...
        Removes the breakpoint first so the continuation runs to completion.
        """
        target, process = self._start()
        target.DeleteAllBreakpoints()
        # Switch to async so Continue() doesn't block if nothing fires.
...
# ---- JS-source breakpoints and stepping --------------------------------
...
class TestJsDebugging(TestBase):
    def _stopped_at_js_breakpoint(self):
        """Stop in wasm (loads the synthetic math.js module), set a JS-file
        breakpoint, then let a second runFactorial() hit it. Returns
        (target, process) stopped at math.js:725."""
        fx = next(f for f in FIXTURES if f["name"] == "factorial")
        # Fire once to hit the wasm breakpoint, then again (via a timer that
        # runs after we resume) to exercise the JS-file breakpoint.
        ...
        # Remove the wasm breakpoint so the next stop is unambiguously the JS one.
    ...
    def test_js_step_advances_one_source_line(self):
        """thread step-over in a JS frame advances by one source line, not by a
        wasm instruction (which jumps an arbitrary distance)."""
        target, process = self._stopped_at_js_breakpoint()
        t = process.GetSelectedThread()
